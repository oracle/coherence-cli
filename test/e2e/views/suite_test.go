...
func TestMain(m *testing.M) {
	...
	var fileName = test_utils.GetFilePath("docker-compose-3-members.yaml")
	...
	if err != nil {
		...
		// wait for balanced services for view cache test
		if err = test_utils.WaitForHttpBalancedServices(context.RestUrl+"/balanced", 120); err != nil {
			fmt.Printf("Unable to wait for balanced services for view tests: %s\n", err.Error())
			exitCode = 1
		} else {
			exitCode = m.Run()
		}
	...
	fmt.Printf("View tests completed with return code %d\n", exitCode)
	if exitCode != 0 {
