...
1. Java 11+ executable on the PATH, if you are using the default Coherence CE 22.06 version.
2. Maven 3.6+ or JDK compatible Gradle version 7.+ executable on the PATH.

NOTE: If you wish to use a later version of Coherence such as CE 22.09, you must use JDK17+.
...
- com.oracle.coherence.ce:coherence:22.06.2
- com.oracle.coherence.ce:coherence-json:22.06.2
...
NOTE: By default, Coherence CE groupId is used and the version is 22.06.2. You can change this via using `-C` for commercial and `-v` to change the Coherence version.
...
2022-08-29 16:00:01.346/0.620 Oracle Coherence 22.06.2 <Info> (thread=main, member=n/a): Loaded operational configuration from "jar:file:/Users/user/.m2/repository/com/oracle/coherence/ce/coherence/22.06.2/coherence-22.06.1.jar!/tangosol-coherence.xml"
...
...
NOTE: If no default-heap is set, then the default of 512m is used unless a value for `-M` is specified.
