...
* <<get-member-channels, `cohctl get member-channels`>> - displays channel details for a topic, service and node
* <<get-subscribers, `cohctl get subscribers`>> - displays subscribers for a topic and service
* <<get-subscriber-channels, `cohctl get subscriber-channels`>> - displays channel details for a topic, service, node and subscriber id
...
NOTE: These topics commands are available to run against Coherence CE editions 22.06.3+, 22.09.1+ 23.03.+ as well as
Coherence Grid Edition 14.1.1.2206.3+.
...
include::../../build/_output/docs-gen/get_member_channels.adoc[tag=text]
...
$ cohctl get member-channels private-messages -s PartitionedTopic -n 4 -c local
...
Display the subscribers channels for a topic, service, node and subscriber.
...
$ cohctl get subscriber-channels  private-messages -s PartitionedTopic -n 4 -S 17663538530 -c local
