...
my-cluster  http  http://127.0.0.1:30000/management/coherence/cluster  14.1.2.0.0  my-cluster    Standalone       N
----
...
CONNECTION  TYPE  URL                                                  VERSION  CLUSTER NAME  CLUSTER TYPE  CTX CREATED
local       http  http://localhost:30000/management/coherence/cluster  21.06.1  my-cluster    Standalone        N
----
...
2. The `CREATED` column indicates if the cluster was a cluster manually created via the `cohctl create cluster` command.

NOTE: If you use the `-o wide` option, an additional column is displayed showing if the management endpoint is running. This
...
and may take a while if you have a large number of connections. See below

[source,bash]
----
$ cohctl get clusters -o wide

CONNECTION  TYPE  URL                                                  VERSION  CLUSTER NAME  CLUSTER TYPE  CTX CREATED RUNNING
local       http  http://localhost:30000/management/coherence/cluster  21.06.1  my-cluster    Standalone        N       Y
----
