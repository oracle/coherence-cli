...
const startupDelayMessage = "startup delay in millis for each server"
const heapMemoryArg = "heap-memory"
const profileArg = "profile"
const startClassArg = "start-class"
...
const startClassMessage = "class name to start server with (default from Coherence version)"
...
func Initialize(command *cobra.Command) *cobra.Command {
	...
	getCmd.AddCommand(getServicesStorageCmd)
